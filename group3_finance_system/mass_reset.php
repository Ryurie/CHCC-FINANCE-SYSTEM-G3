...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['confirm_reset'])) {
    try {
        ...
        // 🔥 CLEANUP: Siguraduhing burado na talaga ang lumang E-Wallet table para hindi maging kalat sa database
        $conn->query("DROP TABLE IF EXISTS student_wallets");

        // NOTE: Hindi natin isasama ang 'fees' at 'students' table para hindi ka na mag-type ulit ng masterlist mo.
        // Pero kung gusto mo silang isama sa burahan, tanggalin ang comment (//) sa ibaba:
        ...
        $conn->query("INSERT INTO audit_logs (admin_name, action_type, description) VALUES ('$admin_name', 'SYSTEM RESET', 'System was completely wiped clean. All transactions were deleted.')");

        $message = "<div class='alert-msg alert-success animate-fade-up'>✅ SYSTEM RESET SUCCESSFUL! All transactions have been permanently deleted. Database is now clean and ready for new entries.</div>";
    } catch (Exception $e) {
...
<head>
    ...
    <style>
        ...
        p { color: #9ca3af; font-size: 1rem; line-height: 1.5; margin-bottom: 20px; }
        
        .warning-text { color: #fca5a5; font-weight: bold; background: rgba(239, 68, 68, 0.1); padding: 15px; border-radius: 8px; font-size: 0.9rem; margin-bottom: 25px; border: 1px solid rgba(239, 68, 68, 0.3);}
        .safe-text { color: #34d399; font-size: 0.85rem; margin-top: 10px; display: block; }

        .btn-nuke { background-color: #ef4444; color: white; border: none; padding: 15px 30px; border-radius: 8px; font-size: 1.1rem; font-weight: bold; cursor: pointer; text-transform: uppercase; width: 100%; transition: 0.3s; margin-bottom: 15px; box-shadow: 0 4px 15px rgba(239, 68, 68, 0.4);}
...
<body>
    <div class="reset-card">
        ...
        <p>You are about to execute a <strong>MASS RESET</strong>. This will permanently wipe out all transaction records.</p>
        
        <div class="warning-text">
            ⚠️ The following will be DELETED:<br>
            • All Invoices & Billing Records<br>
            • All Payments & Receipts<br>
            • All Scholarships & Grants<br>
            • All Audit Logs<br>
            <span class="safe-text">✅ Don't worry, the Masterlist of Students and Fees will NOT be deleted.</span>
        </div>
        
        <form method="POST" action="" onsubmit="return confirm('FINAL WARNING: Are you absolutely sure you want to delete ALL transaction data? This cannot be undone!');">
